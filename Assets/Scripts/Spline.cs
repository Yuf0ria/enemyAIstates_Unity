...
public class Spline : MonoBehaviour
{
    ...
    private Vector3 CalculatePoints(float value01, Vector3 startPos, Vector3 endPos, Vector3 midPos)
    {
        ...
        Vector3 startMiddle = Vector3.Lerp(startPos, midPos, value01),
            middleEnd = Vector3.Lerp(midPos, endPos, value01);
        return Vector3.Lerp(startMiddle, middleEnd, value01); //using lerp to smooth things out
    }
    //calculates the interpolation amount of the spline
    private Vector3 CalculatePosition(float interpolationAmount01)
    ...
    #region CustomCalculations
        public Vector3 CalculatePositonStart(float interpolationAmount01, Vector3 startPos) => 
            CalculatePoints(interpolationAmount01, startPos, _end.position, _middle.position);
        public Vector3 CalculatePositionEnd(float interpolationAmount01, Vector3 endPos) => 
            CalculatePoints(interpolationAmount01, _start.position, endPos,  _middle.position);
    #endregion
