...
public class NpcPointsTp : MonoBehaviour
{
    ...
    #region NavMesh Link Traversal
        private IEnumerator TraverseLink()
        {
            ...
            // Height priority affects traversal behavior:
            // - Going up: slower, no arc (climbing feel)
            // - Going across (jump): normal arc
            // - Going down: faster, small arc
            float duration;
            float heightArc;
            ...
            // Optionally trigger animation states based on direction:
            // animator.SetBool("IsClimbing", isHigherGround);
            // animator.SetBool("IsJumping", isJump && !isHigherGround);
    ...
    #region Height Priority Logic
        // Finds the highest NavMeshLink endpoint within scan radius.
        // Returns null if no suitable higher link is found nearby.
        private NavMeshLink FindHighestNearbyLink()
        {
            NavMeshLink bestLink = null;
            float highestY = transform.position.y + 0.5f; // must be meaningfully higher than current pos
